...
class GoalHandler:
    def __init__(self):
        ...
        # Initialize cancel goal service
        self.cancel_goal_service = rospy.Service('cancel_goal', SetBool, self.handle_cancel_goal)
    ...
    def handle_cancel_goal(self, req):
        if not self.goal_cancelled:
            ...
            return SetBoolResponse(True, "Goal cancelled successfully")
        else:
            rospy.loginfo("No active goal to cancel")
            return SetBoolResponse(False, "No active goal to cancel")
    ...
    def handle_goal_commands(self):
        while not rospy.is_shutdown():
            ...
            if command == 'y':
                ...
                self.handle_cancel_goal(SetBoolRequest(True))
